...
async function swapSections(req, res) {
    try {
        ...
        res.status(200).send("SectionIds swapped");
    } catch (err) {
        res.status(500).send("Database error");
        console.error(err);
    }
...
async function createItem(req, res) {
    try {
        await DB.createItem(req.header("ItemName"), req.header("SectionId"), req.header("Price"));
...
async function editItem(req, res) {
    try {
        await DB.editItem(req.header("ItemId"), req.header("ItemName") || "", req.header("SectionId") || "", req.header("Price") || 0);
